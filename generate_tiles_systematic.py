#!/usr/bin/env python3
"""
generate_tiles_systematic.py — Training tile generator with edge trimming, SYSTEMATIC GRID crops,
...
Output (default):  training_tiles_128_systematic/
...
DEFAULT_OUTPUT_DIR = Path("training_tiles_128_systematic")
...
def sample_positions(
    image_shape: Tuple[int, int],
    pair_name: str,  # kept for API compat, unused (systematic crop is deterministic)
    config: TileConfig,
) -> List[Tuple[int, int]]:
    """Generate systematic grid (y_start, x_start) positions covering the
    reliable zone.

    Strategy: regular grid with stride = tile_size (no overlap for the body),
    plus an "anchored last tile" per row/column whose right/bottom edge is
    aligned with the right/bottom edge of the reliable zone. This guarantees
    every pixel inside the reliable zone is contained in at least one tile.
    The anchored last tile may overlap with its predecessor (typically by
    30-50 pixels); this is intentional and is the cost of full coverage.

    The ``tiles_per_pair`` config option is ignored in systematic mode — the
    number of tiles per pair is determined entirely by image size and
    tile_size / edge_trim parameters.
    """
    h, w = image_shape
    trim, tile = config.edge_trim, config.tile_size
    y_max = h - trim - tile  # last valid y_start (inclusive)
    x_max = w - trim - tile

    if y_max < trim or x_max < trim:
        LOG.error("Image %dx%d too small for tiling (trim=%d, tile=%d)",
                  w, h, trim, tile)
        return []

    # Body of grid: stride = tile_size starting from trim
    y_starts = list(range(trim, y_max + 1, tile))
    x_starts = list(range(trim, x_max + 1, tile))

    # Anchor last tile to bottom/right edge of reliable zone (if not already)
    if y_starts[-1] != y_max:
        y_starts.append(y_max)
    if x_starts[-1] != x_max:
        x_starts.append(x_max)

    LOG.info("[%s] systematic grid: %d rows x %d cols = %d tiles",
             pair_name, len(y_starts), len(x_starts),
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="(Ignored in systematic mode — kept for API compatibility.)",
    )
    parser.add_argument(
