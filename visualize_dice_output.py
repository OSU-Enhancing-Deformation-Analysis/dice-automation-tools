...
def print_data_summary(df, filepath):
    ...
    if 'DISPLACEMENT_X' in df.columns and 'DISPLACEMENT_Y' in df.columns:
        ...
        print(f"  Displacement X: min={dx.min():.3f}, max={dx.max():.3f}, mean={dx.mean():.3f}")
        print(f"  Displacement Y: min={dy.min():.3f}, max={dy.max():.3f}, mean={dy.mean():.3f}")
        print(f"  Magnitude: min={mag.min():.3f}, max={mag.max():.3f}, mean={mag.mean():.3f}")
...
Examples:
  ...
  %(prog)s DICe_solution_050.txt --no-background -o output.png
    ...
    parser.add_argument('--strain-component', default='VSG_STRAIN_XX',
                        choices=['VSG_STRAIN_XX', 'VSG_STRAIN_YY', 'VSG_STRAIN_XY'],
    ...
    if args.strain:
        ...
        plot_displacement_vectors(df, background, args.output, args.scale, title)
