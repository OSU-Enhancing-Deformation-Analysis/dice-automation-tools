...
def analyze_single_file(filepath, verbose=False):
    ...
    if verbose:
        ...
        print(
            f"SIGMA - mean: {stats['sigma_mean']:.4f}, std: {stats['sigma_std']:.4f}")
        print(
            f"GAMMA - mean: {stats['gamma_mean']:.4f}, range: [{stats['gamma_min']:.4f}, {stats['gamma_max']:.4f}]")
...
def analyze_sequence(dice_dir, output_file=None):
    ...
    print(f"\nMATCH Statistics:")
    print(
        f"  Overall success rate: {stats_df['match_success_rate'].mean():.1f}%")
    print(
        f"  Frames with 100% success: {(stats_df['match_success_rate'] == 100).sum()}")
    print(
        f"  Frames with 0% success: {(stats_df['match_success_rate'] == 0).sum()}")
    ...
    print(
        f"  Range: [{combined_df['SIGMA'].min():.6f}, {combined_df['SIGMA'].max():.6f}]")
    ...
    print(
        f"  Range: [{combined_df['GAMMA'].min():.4f}, {combined_df['GAMMA'].max():.4f}]")

    # Quality filtering recommendations
    print("\n" + "=" * 60)
    print("ML TRAINING DATA RECOMMENDATIONS")

    # Alternative quality filter (ignoring MATCH, which may be unreliable for
    # SEM images)
    ...
    print(
        f"Alternative filter (GAMMA<0.5, SIGMA<0.02, ignoring MATCH): {len(alt_quality)} ({len(alt_quality)/len(combined_df)*100:.1f}%)")

    # Points with good GAMMA only
    good_gamma = combined_df[combined_df['GAMMA'] < 0.3]
    print(
        f"Good matching quality (GAMMA<0.3): {len(good_gamma)} ({len(good_gamma)/len(combined_df)*100:.1f}%)")
    ...
    print(
        f"High-quality points (MATCH=0, GAMMA<0.5, SIGMA<0.02): {len(high_quality)} ({len(high_quality)/len(combined_df)*100:.1f}%)")
...
def main():
    ...
    parser.add_argument(
        'dice_dir',
        help='Directory containing DICe solution files')
    parser.add_argument(
        '-o',
        '--output',
